...
import styledToast from "src/components/core/StyledToast";
...
const EditedFormInput= ({
    ...
    return(
        <FormInput
                ...
                rightElementTextStyle={{fontSize:'12px', height:'32px', width:'fit-content', color:'#666666'}}
                variant={"unstyled"} border={"0"} borderBottom="1px solid #666666" borderRadius={0} px="0" py="1" style={{height:'32px', color:color}} bg="transparent" fontSize={"sm"} placeholder="$5,500"
                type="number"
            />
    )
}
...
export default function CompensationForm(){
    ...
    const handleSubmit = async (data: FormInputOptions) => {
        ...
        styledToast({status:"success", description:"Compensation has been updated successfully", toast:toast})
    };
    ...
    return(
        <Flex flexDirection={"column"} borderRadius={"15px"} border={"1px solid"} borderColor="bordergrey" p='4' bg={'white'} flex="1" marginInlineStart="0" >
            <Text fontWeight="bold" fontSize="18px" mb="4" >Compensation Details</Text>
            {renderForm(
            <Stack fontSize="sm" textTransform={"capitalize"} spacing={"4"} >
              ...
              <Stack pt="6">
              <Button variant={"darkBtn"} w="100%" mt="10" py="15px" type="submit" >Update Compensation</Button>
              </Stack>
            </Stack>
            )}
          </Flex>
